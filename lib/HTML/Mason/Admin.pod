...
the same as the server's C<DocumentRoot>. (See CONFIGURING VIRTUAL
...
parameters for the B<HTML::Mason::Parser>,
...
See the Multiple component roots section for more details on
...
argument handling.  Please see HTML::Mason::ApacheHandler/PARAMETERS TO THE use() DECLARATION
for more details.
...
B<Storable>). Mason's C<Config.pm> controls which packages are used.
...
authenticated users, and in C<data_dir/debug/anon> for anonymous
users.

Debug files can only be used with the C<handler.pl> configuration method. 

Several ApacheHandler parameters are required to activate and
configure debug files:
...
want to omit certain flags (like preloading) in C<handler.pl> when
