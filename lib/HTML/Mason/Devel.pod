...
Two global per-request objects are available to all components: C<$r> and
C<$m>.

C<$r>, the mod_perl request object, provides a Perl API to the current
...
C<$m>, the Mason request object, provides an analogous API for
Mason. Almost all Mason features not activated by syntactic tags are
accessed via C<$m> methods.  You'll be introduced to these methods
...
get a handle on a component object from request methods like C<< $m->current_comp >> and C<< $m->fetch_comp >>.
...
because they may not be destroyed until the first time the C<< <%shared> >>
section executes in a future request.  C<< <%init> >>
...
Under ApacheHandler and CGIHandler, Mason automatically sends HTTP
headers, via C<< $r->send_http_header() >> with ApacheHandler, and via
C<< CGI->header() >> with CGIHandler, but it will not send headers if
they've already been sent manually.
...
The administrator can change the default automatic header behavior via
the L<auto_send_headers|HTML::Mason::Params/auto_send_headers> parameter. You can also turn it off on
...
When running under ApacheHandler, the value returned from the top-most
component becomes the status code of the request. If no value is
...
Note that when running under CGIHandler, the value you return from
your components, or via C<< $m->abort() >>, is not able to affect the
...
Second, if you're running under mod_perl with ApacheHandler, you can
use Apache's internal_redirect method, which works whether or not the
new URL will be handled by Mason.  Use it this way:
